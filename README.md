...
# leyoai-analytics-small

## 中文介绍

LeyoAI Analytics Model (Small) 是一个数据分析助手，基于 Qwen2.5-1.5B-Instruct 微调，平衡了分析深度和响应速度。
...
model = AutoModelForCausalLM.from_pretrained(
    "FFZwai/leyoai-analytics-small",
    device_map="auto"
)
tokenizer = AutoTokenizer.from_pretrained("FFZwai/leyoai-analytics-small")

# Inference
prompt = "使用 Python 生成这组数据的可视化图表"
...
LeyoAI Analytics Model (Small) is a data analytics assistant fine-tuned from Qwen2.5-1.5B-Instruct, balancing analysis depth and response speed.
...
# Inference
prompt = "Generate visualization charts for this dataset using Python"
inputs = tokenizer(prompt, return_tensors="pt").to(model.device)
outputs = model.generate(**inputs, max_length=100)
response = tokenizer.decode(outputs[0], skip_special_tokens=True)
print(response)
